...
# ╔═╡ 70ce5d92-3453-11ed-28a6-35aff8170fb0
using GLMakie:streamplot,Point2

# ╔═╡ e148d2c0-26ee-4943-937f-1c51b66d2106
using PlutoUI,DifferentialEquations, Plots; theme(:gruvbox_dark)

# ╔═╡ 6c6c43ef-89c5-487c-8b2a-06c9f05e0ea2
md"""#### Sliders y streamplots (o similares)"""

# ╔═╡ 909f1465-5ae0-4317-80c6-7491638ba4e6
md"""La siguiente función recibe una función `f=f(x,y,μ)`, donde μ es un valor del parámetro que define el sistema y un vector `M` con valores para el parámetro. Devuelve un vector que contiene todos los streamplots correspondientes a `f` para cada valor de `M`."""

# ╔═╡ edc41bc4-254a-4805-89af-8b6e7c84c2e9
function compute_movie(f,M)
	v = []
	rx = -1:1 #intervalo a graficar en x
	ry = -1:1 #intervalo a graficar en y
	for μ in M
		push!(v,streamplot((x,y)->f(x,y,μ),rx,ry,title="μ=$μ"))
	end
	v
end

# ╔═╡ 2fc825f0-5dac-4e81-a16a-e0c2d1a2c4e3
begin
	M        = -1:0.1:1
	F(x,y,μ) = Point2(y+μ*x−x*y^2, μ*y−x−y^3)
	movie    = compute_movie(F,M)
end

# ╔═╡ 1169d3b1-1ba0-4c30-ac2d-a6569bec9fe6
md"""Slider que recorre el rango de `M`"""

# ╔═╡ 7f83be90-d707-4bdd-88b3-0029399eb65a
@bind μ Slider(1:length(M))

# ╔═╡ 94a42e7b-2677-46ad-b2e5-29cee3625626
M[μ]

# ╔═╡ da77c5c8-00b6-4f1c-a9b9-27a92808f9ed
movie[μ]

# ╔═╡ 2038ac16-b87a-4dd7-8fdb-0ae79fe59480
md"""Alternativamente podemos calcular muchas soluciones del sistema"""

# ╔═╡ b4330f79-fe1e-478c-a24f-1cc31684c13b
function compute_movie_sols(modelo,xlims,ylims,P,U₀)
	tspan = (0.0,100.0)
	m     = Int(length(U₀)/2)
	peli  = []
	for p in P
		prob  = ODEProblem(modelo,U₀,tspan,p)
		sol   = solve(prob)
		frame = plot(sol,idxs=[(i,i+1) for i in 1:2:m-1], legend=:none,xlims=xlims,ylims=ylims)
		push!(peli,frame)
	end
	peli
end

# ╔═╡ cc5d23ef-429a-44ca-aa46-a42c83f73945
function modelo!(du,u,p,t)
	for i in 1:2:Int(length(u)/2)
		du[i]   = u[i+1]+p*u[i]−u[i]*u[i+1]^2
		du[i+1] = p*u[i+1]−u[i]−u[i+1]^3
	end
end

# ╔═╡ 608ab0f2-b0e4-44cd-8e38-d247775be3b3
begin
	P  = -1:0.1:1
	u₀ = 2rand(200) .- 1.0 
	peli = compute_movie_sols(modelo!,(-1,1),(-1,1),P,u₀)
end

# ╔═╡ 7cf0fa97-ad0e-4863-a317-3950857dd8c8
@bind ind Slider(1:length(P))

# ╔═╡ 9ec88bf7-409a-4c0d-98c0-4339581daab2
P[ind]

# ╔═╡ 34b46263-c8d2-404a-8eb2-120019d1ab0d
peli[ind]
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "39169b2176aa4b9ec6a82d4192f2a4967144c3b6"

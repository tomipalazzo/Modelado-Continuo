...
# ╔═╡ 93803942-3533-11ed-2d7d-9fa951dfe9cd
using DifferentialEquations,OrdinaryDiffEq,LinearAlgebra,Unitful, RecursiveArrayTools,Plots;theme(:dark)

# ╔═╡ d8c6b0be-1d6b-4d90-bf17-49903e2e9c1a
md"""### Sistema solar"""

# ╔═╡ c171ad16-a310-490b-b7ef-4f13e835ecda
const G = uconvert(u"km^3/(kg*s^2)",6.673e-11u"N*m^2/kg^2")

# ╔═╡ 503e47c8-13b5-4cfb-8399-b752faade1cf
masas = 1e24*[1988500.0,0.3302,48.685,5.97219,0.07349,0.64171,1898.18722,568.34,86.813,102.409]u"kg"

# ╔═╡ e8bcf967-82f8-4956-8e82-6016beda6b6a
radio = [696500,2440,6051.893,6378.137,1737.53,3396.19,71492,60268,25559,24766]u"km"

# ╔═╡ 580796b1-8db5-4af7-a558-c696cebf2a85
md"""Datos de posición y velocidad a las 0hs del 15/09/22, tomados de [acá](https://ssd.jpl.nasa.gov/horizons/app.html#/)."""

# ╔═╡ 9010e32c-47fd-44af-aa19-b3ec11576a92
begin 
	S   = [0.0,0.0,0.0]u"km"
	vS  = [0.0,0.0,0.0]u"km/s"
	Me  = [4.897164397379762E+07, -3.424020982565113E+07, -7.290086780041797E+06]u"km"
	vMe = [1.832605492353993E+01, 4.217261071471366E+01,  1.765346083977468E+00]u"km/s"
	V   = [-9.086438403903614E+07,  5.714177637210152E+07,  6.027535166735020E+06]u"km"
	vV  = [-1.878600830892175E+01,-2.981751671845472E+01, 6.746527000792568E-01]u"km/s"
	T   = [1.489405049608335E+08, -2.136411904698220E+07,  2.352711679693311E+02]u"km"
	vT  = [3.753282579070275E+00, 2.936606012233193E+01, -2.599562093113406E-03]u"km/s"
	L   = [1.491966891303162E+08, -2.106772446604900E+07,  3.182663465494290E+03]u"km"
	vL  = [3.037258528078988E+00,  3.006094492014939E+01, 8.684627163162695E-02]u"km/s"
	Ma  = [1.867383025728507E+08,  1.043324124249760E+08, -2.393996970013395E+06]u"km"
	vMa = [-1.089226966098709E+01,2.322376108805311E+01,7.539142025969117E-01]u"km/s"
	J   = [7.405241839811879E+08, 3.089449364291938E+07,-1.669623824391358E+07]u"km" 
	vJ  = [-6.995801602236956E-01,1.368059497135384E+01,-4.126159013328046E-02]u"km/s"
	Sa  = [1.171661837418448E+09,-8.959856875774095E+08,-3.104918354469573E+07]u"km"
	vSa = [5.331248737698290E+00,7.668443045080663E+00,-3.460230809568863E-01]u"km/s"
	U   = [2.046294742920606E+09,2.117790721752401E+09,-1.865674039328671E+07]u"km"
	vU  = [-4.948545213924935E+00,4.428723591853194E+00,8.025988679018692E-02]u"km/s"
	N   = [4.447270503039445E+09,-4.906566057222244E+08,-9.238996648158738E+07]u"km"
	vN  = [5.612757907772880E-01,5.450869538037618E+00,-1.250865272094792E-01]u"km/s"
	u₀  = ArrayPartition(S,Me,V,T,L,Ma,J,Sa,U,N,vS,vMe,vV,vT,vL,vMa,vJ,vSa,vU,vN)
end

# ╔═╡ f21c33e1-3d8c-4d51-a004-95e83c348cbe
function planets!(du,u,m,t)
	n = Int(length(u)/6)
	du[3n+1:end] .= 0.0u"km/s^2"
	for i in 1:n
		for k in 1:3
			du.x[i][k]   = u.x[n+i][k]
		end
		for j in i+1:n
			dij        = norm(u.x[i]-u.x[j])
			for k in 1:3
				du.x[n+i][k] += G*m[j]*(u.x[j][k]-u.x[i][k])/dij^3
				du.x[n+j][k] += G*m[i]*(u.x[i][k]-u.x[j][k])/dij^3
			end
		end
	end	
end

# ╔═╡ 8d652ade-004b-4250-9edf-e8127924b234
begin
	tspan = (0.0u"d",365*100u"d")
	prob = ODEProblem(planets!,u₀,tspan,masas)
end

# ╔═╡ 891fb944-7bd1-4750-8170-29855bf029d3
begin
	const n     = Int(length(u₀)/6)
	const units = ArrayPartition([ones(3)u"m" for i in 1:n]...,[ones(3)u"m/s" for i in 1:n]...)
	sol = solve(prob,Feagin14(),abstol=1e-14*units,reltol=1e-14*units)
end

# ╔═╡ 2688cfb7-d4e9-440a-aed1-df7f537d16a1
md"""Animación de el sistema: Sol, Mercurio, Venus, Tierra, Luna, Marte, Júpiter, Saturno, Urano, Neptuno, por 100 años."""

# ╔═╡ bbbf1cb2-f476-4e0d-9b88-3fcf370e0f29
begin
	time = (tspan[1]:20u"d":tspan[2])
	pal  = palette(:Spectral,15)
	cols = pal[[6,12,3,15,8,2,13,4,14,10]]
	theme(:gruvbox_dark)
	anim = @animate for t in time
		u = ustrip.(sol(t))
		pos_x = [u.x[i][1] for i in 1:n]
		pos_y = [u.x[i][2] for i in 1:n]
		#pos_z = [u.x[i][3].-u.x[1][3] for i in 1:n]
		L     = 4.5e9
		tams  = [2.1,1.4,1.8,1.8,1,1.7,2,1.8,1.6,1.6]
		scatter(pos_x,pos_y,xlims=(-L,L),ylims=(-L,L),
				c = cols,
				markersize=tams,legend=:none,
				markerstrokewidth=0.0)
	end
	mp4(anim,"sistema_solar_100.mp4",fps=48)
end
		

# ╔═╡ 63a21319-61c2-4495-b7ae-9dc7cb3ac53b
md"""Los colores son una selección de estos:"""

# ╔═╡ 6f4c496d-df6f-45d9-92ef-1f569d9ef1c0
palette(:Spectral,15)

# ╔═╡ 4b2dbbed-08f6-4640-bda3-acf087d695b2
md"""Animación hasta Marte (Luna incluida) por 5 años"""

# ╔═╡ c7ef2f2b-f877-4d3c-9839-a4e3fd0eb89d
begin
	time2 = (tspan[1]:1u"d":5*365.0u"d")
	names = ["Sol", "Mercurio", "Venus", "Tierra", "Luna", "Marte"]
	anim2 = @animate for t in time2
		u = ustrip.(sol(t))
		pos_x = [u.x[i][1] for i in 1:6]
		pos_y = [u.x[i][2] for i in 1:6]
		#pos_z = [u.x[i][3].-u.x[1][3] for i in 1:n]
		L     = 2.5e8
		tams  = [10,1.5,3,3,1.1,2.8]
		scatter(pos_x,pos_y,xlims=(-L,L),ylims=(-L,L),
				c = cols[1:6],
				markersize=tams,legend=:none,
				markerstrokewidth=0.0)
		for k in 1:6
			annotate!(pos_x[k]+1.5e7,pos_y[k]+(-1)^(k==5)*1e7,text(names[k],cols[k],6))
		end
	end
	mp4(anim2,"hasta_marte_5.mp4",fps=24)
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "1122a08e43b889c3e9e97692d54dfb35a1ccdc06"

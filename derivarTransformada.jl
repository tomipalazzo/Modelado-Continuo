...
# ╔═╡ b30f1294-53ae-11ed-3b97-35059e3701fa
begin
	using Plots
	using PlutoUI
	using FFTW
...
# ╔═╡ 72ff54b8-3a41-4fdf-a54c-5b708c7d4520
md"""
# Ejercicio 25
"""

# ╔═╡ 3b3aafcf-8990-4f2f-bc60-c44b42e34d21
function derivarTransformada(f;exponente=10, L=(0,2π))
	N = 2^exponente
	K = fftshift((-N/2:N/2)[1:end-1]) /(L[2]-L[1])
	t = (L[1]:(L[2]-L[1])/N:L[2])[1:end-1]

	f̂′ = 2*π * 1im * K.*fft(f.(t))
	
	return ifft(f̂′)
end

# ╔═╡ 862ab4ff-2633-4841-81c1-eae49c868531
begin 
	f(x) = sin(x)
	d = derivarTransformada(f)
	L=(0,2π)
	tt = (L[1]:(L[2]-L[1])/2^10:L[2])[1:end-1]

	
	plot(tt, real.(d))
end

# ╔═╡ edf01473-8380-43eb-9c8a-ce95f6cf0977


# ╔═╡ f7e6242f-52ec-4123-8046-bab4adfcb219
